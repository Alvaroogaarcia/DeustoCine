...
public class CompraEntradas extends JFrame{
	...
	public CompraEntradas() {
		...
		// Panel central con las tarjetas de sesiones
		panelCartelera = new JPanel();
		panelCartelera.setLayout(new GridLayout(0, 3, 20, 20));
	...
	//Carga las sesiones de la BD y las muestra en el panel
	private void cargarSesiones(String textoTitulo) {
		panelCartelera.removeAll();
	
		List<domain.Sesion> listaSesiones = sesionDAO.listar();
		for (domain.Sesion s : listaSesiones) {
			...
			if (textoTitulo != null && !textoTitulo.isBlank()) {
				...
				if (!titulo.contains(textoTitulo.toLowerCase())) {
					continue;
				}
			}
	...
	//Carga las sesiones filtrando por genero de la pelicula
	private void cargarSesionesPorGenero(String genero) {
		...
		for (domain.Sesion s : listaSesiones) {
			...
			if (p.getGenero() != null && p.getGenero().equalsIgnoreCase(genero)) {
				panelCartelera.add(crearPanelSesion(s, p));
			}
		}
	...
	//Crea la tarjeta de una sesion
	private JPanel crearPanelSesion(domain.Sesion sesion, domain.Pelicula pelicula) {
		...
		JLabel lblInfo = new JLabel(textoHtml, SwingConstants.CENTER);
	...
	//Carga una imagen desde la carpeta de imagenes y la escala
	private ImageIcon cargarImagen(String nombreImagen, int ancho, int alto) {
		...
		String ruta = RUTA_IMAGENES + nombreImagen;
		ImageIcon iconoOriginal = new ImageIcon(ruta);
	
		if (iconoOriginal.getIconWidth() <= 0 || iconoOriginal.getIconHeight() <= 0) {
			System.err.println("No se ha podido cargar la imagen: " + ruta);
			return null;
		}
	...
	private void mostrarVentanaDetalle(domain.Sesion sesion, domain.Pelicula pelicula) {
		...
		JLabel lblTexto = new JLabel(sb.toString());
		panelContenido.add(lblTexto, BorderLayout.CENTER);
